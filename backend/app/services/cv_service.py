...
try:
    from awesomecv_jinja import render, render_pdf
...
def _build_canonical_prompt(resume_text: str) -> str:
    return (
        ...
        "    \"links\": [string],\n"
        "    \"experience\": [\n"
        "      {\"id\": string, \"title\": string, \"organization\": string, \"location\": string or null, \"period\": string or null, \"bullets\": [{\"id\": string, \"text\": string, \"source_id\": string or null}]}\n"
        "    ],\n"
        "    \"education\": [\n"
        "      {\"id\": string, \"degree\": string, \"institution\": string or null, \"location\": string or null, \"period\": string or null, \"bullets\": [{\"id\": string, \"text\": string, \"source_id\": string or null}]}\n"
        "    ],\n"
        "    \"skills\": [\n"
        "      {\"id\": string, \"category\": string, \"items\": [string]}\n"
        "    ],\n"
        "    \"projects\": [\n"
        "      {\"id\": string, \"name\": string, \"role\": string or null, \"period\": string or null, \"description\": string or null, \"bullets\": [{\"id\": string, \"text\": string, \"source_id\": string or null}]}\n"
        "    ],\n"
        "    \"certificates\": [\n"
        "      {\"id\": string, \"title\": string, \"issuer\": string or null, \"year\": string or null}\n"
        "    ],\n"
        "    \"publications\": [\n"
        "      {\"id\": string, \"title\": string, \"venue\": string or null, \"year\": string or null, \"notes\": string or null}\n"
        "    ],\n"
        "    \"languages\": [\n"
        "      {\"id\": string, \"name\": string, \"level\": string or null}\n"
        "    ],\n"
        "    \"awards\": [\n"
        "      {\"id\": string, \"title\": string, \"issuer\": string or null, \"year\": string or null}\n"
        "    ]\n"
...
def parse_resume_to_canonical(*, resume_text: str, model: str, lm_timeout: float | None = None) -> CvCanonicalData:
    effective_timeout = lm_timeout if lm_timeout is not None else DEFAULT_LM_TIMEOUT
    prompt = _build_canonical_prompt(resume_text)
    json_schema = {
        "type": "object",
        "properties": {
...
def generate_cv_pdf(*, resume_text: str, job_title: str, company: str | None, job_description: str, model: str, doc_type: str, template_id: str, lm_timeout: float | None = None) -> bytes:
    if template_id != DEFAULT_TEMPLATE_ID:
        raise ValueError(f"Unsupported template_id: {template_id}")
    if doc_type not in ALLOWED_DOC_TYPES:
        raise ValueError(f"Unsupported doc_type: {doc_type}")

    canonical = parse_resume_to_canonical(resume_text=resume_text, model=model, lm_timeout=lm_timeout)
...
def render_cv_pdf_from_payload(*, payload: dict, doc_type: str) -> bytes:
    ...
    with tempfile.TemporaryDirectory(dir=tmp_root) as tmp_dir:
        ...
        if DEBUG_TEX:
            ...
            render(payload, output=str(tex_path), doc_type=doc_type)
        ...
        render_pdf(payload, output=str(output_path), doc_type=doc_type)
